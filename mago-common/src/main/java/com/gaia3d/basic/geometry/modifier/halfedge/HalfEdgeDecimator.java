...
public class HalfEdgeDecimator extends HalfEdgeModifier {
        ...
        while (!finished && iteration < maxIterations) {
            ...
            for (int i = 0; i < halfEdgesCount; i++) {
                ...
                if (halfEdge.hasTwin() && positionType == PositionType.INTERIOR) {
                    if (collapseHalfEdge(halfEdge, i, vertexAllOutingEdgesMap, mapVertexToSamePosVertices, maxDiffAngDeg, frontierMaxDiffAngDeg, hedgeMinLengthCurrent,
    ...
    public boolean collapseHalfEdge(HalfEdge halfEdge,
        ...
        if (halfEdge.getLength() > hedgeMinLength) {
            if (!HalfEdgeDecimaterUtils.decideIfCollapseCheckingFaces(halfEdge, vertexAllOutingEdgesMap, mapVertexToSamePosVertices, maxDiffAngDeg, maxAspectRatio, smallHedgeSize)) {
                return false;
            }
        }
