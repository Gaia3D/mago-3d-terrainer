...
public class ChemicalContaminationDataConverter {
	...
	"legend" : [
        ...
        // now, read the data
        this.dataContainer.year = objectNodeRoot.get("year")
                .asInt();
        this.dataContainer.month = objectNodeRoot.get("month")
                .asInt();
        this.dataContainer.day = objectNodeRoot.get("day")
                .asInt();
        this.dataContainer.hour = objectNodeRoot.get("hour")
                .asInt();
        this.dataContainer.minute = objectNodeRoot.get("minute")
                .asInt();
        this.dataContainer.second = objectNodeRoot.get("second")
                .asInt();
        this.dataContainer.millisecond = objectNodeRoot.get("millisecond")
                .asInt();

        ObjectNode centerGeoCoordNode = (ObjectNode) objectNodeRoot.get("centerGeographicCoord");
        this.dataContainer.geoCoordCenterLongitudeDeg = centerGeoCoordNode.get("longitude")
                .asDouble();
        this.dataContainer.geoCoordCenterLatitudeDeg = centerGeoCoordNode.get("latitude")
                .asDouble();
        this.dataContainer.geoCoordCenterAltitude = centerGeoCoordNode.get("altitude")
                .asDouble();
        this.dataContainer.width_km = objectNodeRoot.get("width_km")
                .asDouble();
        this.dataContainer.height_km = objectNodeRoot.get("height_km")
                .asDouble();
        ...
        if (legendNode != null && legendNode.isArray()) {
            ...
            for (int i = 0; i < legendCount; i++) {
                ...
                int index = legendObjectNode.get("index")
                        .asInt();
                double value = legendObjectNode.get("value")
                        .asDouble();
                ArrayNode rgbaArrayNode = (ArrayNode) legendObjectNode.get("rgba");
                double r, g, b, a;
                r = rgbaArrayNode.get(0)
                        .asDouble();
                g = rgbaArrayNode.get(1)
                        .asDouble();
                b = rgbaArrayNode.get(2)
                        .asDouble();
                a = rgbaArrayNode.get(3)
                        .asDouble();
        ...
        if (isoValuesNode != null && isoValuesNode.isArray()) {
            ...
            for (int i = 0; i < isoValuesCount; i++) {
                isoValuesList.add(isoValuesArrayNode.get(i)
                        .asDouble());
            }
        }
        ...
        for (int layer = 0; layer < layersCount; layer++) {
            ...
            String folderPath = objectLayersNode.get("folderPath")
                    .asText();
            ...
            dataLayer.minAltitude = objectLayersNode.get("minAltitude")
                    .asDouble();
            dataLayer.maxAltitude = objectLayersNode.get("maxAltitude")
                    .asDouble();
            ...
            if (layer == 0) {
                ...
                if (timeSlicesCount != dataLayer.timeSlicesFileNames.size()) {
                    log.info("ERROR *** *** in ConvertDataByDataStructureFile");
                    return;
                }
            }
        }
...
9 = 1.12E8
    ...
    private void convertDataByDateMarchingCubes(TreeMap<Double, DataSlice> treeMapVolumeData, String outputFolderPath, double[] isoValuesArray, int idx) {
        ...
        if (gaiaSceneMaster != null) {
            ...
            GaiaNode rootNode = gaiaSceneMaster.getNodes()
                    .get(0);
...
//        this.dataContainer.getGeoCoordBoundingBox(bbox);
    ...
    private VoxelCPGrid3D makeVoxelCPGrid3D(TreeMap<Double, DataSlice> treeMapVolumeData, boolean addLastTopSlice) {
        ...
        DataSlice firstDataSlice = treeMapVolumeData.get(keys.iterator()
                .next());
...
//            double latDegStep = latDegRange / sliceRowCount;
            ...
            for (int row = 0; row < sliceRowCount; row++) {
                for (int col = 0; col < sliceColCount; col++) {
                    ...
                    voxelCp.getPosition()
                            .set(localPosX, localPosY, maxAlt);
                    int hola = 0;
                }
            }
...
//            resultMosaicTexture.data[i * 4 + 3] = (byte)200;
    ...
    public void ConvertMatrixAsciTxtToJsonByDataStructureFile(String inputDataStructurePath, String outputFolderPath) {
        ...
        // now, read the data
        this.dataContainer.date = objectNodeRoot.get("date")
                .asText();
        ObjectNode centerGeoCoordNode = (ObjectNode) objectNodeRoot.get("centerGeographicCoord");
        this.dataContainer.geoCoordCenterLongitudeDeg = centerGeoCoordNode.get("longitude")
                .asDouble();
    ...
    private void ConvertMatrixAsciTxtToJson(String inputFolderPath, String outputFolderPath, String inputFileName) {
        try {
            log.info("Converting : " + inputFileName + " to json.");
            ...
            log.info("ERROR *** *** in ConvertMatrixAsciTxtToJson");
            log.error("Error : ", e);
        }
    }
}
